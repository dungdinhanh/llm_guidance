...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        fake_data2 = ["bllurry figures, lowres, text, error, cropped, worst quality, low quality, jpeg artifacts, ugly, duplicate, morbid, mutilated, out of frame, extra fingers, mutated hands, poorly drawn hands"]
        if self.tokenizer is not None:
            fake_data2 = self.tokenizer(fake_data2).squeeze()

        sample = dict(real=real_data, fake=fake_data, fake2=fake_data2)
        return sample
...
def calculate_clip_score(dataloader, model, real_flag='img', fake_flag='txt'):
    ...
    for batch_data in tqdm(dataloader):
        ...
        fake2= batch_data['fake2']
        fake2_features = forward_modality(model, fake2, fake_flag)
        ...
        fake2_features = fake2_features / fake_features.norm(dim=1, keepdim=True).to(torch.float32)
        # calculate scores
        # score = logit_scale * real_features @ fake_features.t()
        # score_acc += torch.diag(score).sum()
        score =  (fake_features * real_features).sum()

        score2 = (fake2_features * real_features).sum()
        # print(logit_scale)
    ...
    return score_acc / sample_num, score_neg/sample_num
...
def main():
    ...
    clip_score, clip_neg = calculate_clip_score(dataloader, model)
    clip_score = clip_score.cpu().item()
    clip_neg = clip_neg.cpu().item()
    print('CLIP Score: ', clip_score)
    print('CLIP Negative:', clip_neg)
    clip_file = os.path.join(os.path.dirname(args.ref_path), "clip.csv")
    f = open(clip_file, "w")
    f.write("CLIP, CLIP negative\n")
    f.write(f"{clip_score, clip_neg},\n")
    f.close()
