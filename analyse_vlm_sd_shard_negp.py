...
def main():
    ...
    analysis_folder_dir = os.path.join(sample_dir, "analysis")
    ...
    print(f"Saving analysis in {analysis_folder_dir}")
    ...
    caption_loader = [["A massive, ancient Gothic castle on a cliff under a full moon, with eerie, misty surroundings. Gargoyles perch on turrets, and faint candlelight flickers in the castle windows. Dense forest encircles the structure, adding a dark, ominous atmosphere."]]
    caption_iter = iter(caption_loader)
    # setup pipe
    cache_hub_folder = os.path.join(base_folder, "hub")
    os.makedirs(cache_hub_folder, exist_ok=True)
    pipe = StableDiffusionLVMPipelineUpgradedNegP.from_pretrained("CompVis/stable-diffusion-v1-4", cache_dir=cache_hub_folder, device_map="balanced")
    ...
    current_samples=0
    # accelerator.wait_for_everyone()
    process_prompt_code = args.prompt_process
    if process_prompt_code == 0:
        process_prompt_mode = WKWN
    else:
        process_prompt_mode = WDTN
    for i in range(iterations):
        ...
        samples = pipe(prompt=prompts, guidance_scale=args.cfg_scale, skip=args.skip, lvm_tokenizer=tokenizer, lvm_model=model, 
                       lvm_image_processor=image_processor, do_lvm_guidance=args.lvm_guidance,
                       track_image_prompt=True, analysis_folder=analysis_folder_dir, process_prompt_mode=process_prompt_mode).images
...
def create_argparser():
    ...
    parser.add_argument("--prompt_process", type=int, choices=[0, 1])
    return parser
