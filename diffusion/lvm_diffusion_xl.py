...
class StableDiffusionXLLVMPipeline(StableDiffusionXLPipeline):
    def __init__(self, vae, text_encoder, text_encoder_2, tokenizer, tokenizer_2, 
                 unet, scheduler, image_encoder = None, feature_extractor = None, 
                 force_zeros_for_empty_prompt = True, add_watermarker = None):
        super().__init__(vae, text_encoder, text_encoder_2, tokenizer, 
                         tokenizer_2, unet, scheduler, image_encoder, feature_extractor, 
                         force_zeros_for_empty_prompt, add_watermarker)
    ...
    def __call__(
        ...
        analysis_folder = "./analysis", process_prompt_mode=WKWN , skip=5,
        **kwargs,
    ):
        r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if lvm_guidance or track_image_prompt:
                    if i % skip == 0:
                        latents_x0 = copy.deepcopy(latents)
                        latents_x0 = latents_x0.to(latents.device)
                        ...
                        if needs_upcasting:
                            self.upcast_vae()
                            latents_x0 = latents_x0.to(next(iter(self.vae.post_quant_conv.parameters())).dtype)
                        elif latents_x0.dtype != self.vae.dtype:
                            if torch.backends.mps.is_available():
                                # some platforms (eg. apple mps) misbehave due to a pytorch bug: https://github.com/pytorch/pytorch/pull/99272
                                self.vae = self.vae.to(latents_x0.dtype)
                        ...
                        if has_latents_mean and has_latents_std:
                            latents_mean = (
                                torch.tensor(self.vae.config.latents_mean).view(1, 4, 1, 1).to(latents_x0.device, latents_x0.dtype)
                            )
                            latents_std = (
                                torch.tensor(self.vae.config.latents_std).view(1, 4, 1, 1).to(latents_x0.device, latents_x0.dtype)
                            )
                            latents_x0 = latents_x0 * latents_std / self.vae.config.scaling_factor + latents_mean
                        else:
                            latents_x0 = latents_x0 / self.vae.config.scaling_factor

                        images_x0 = self.vae.decode(latents_x0, return_dict=False)[0]
                        ...
                        if track_image_prompt:
                            #save eh
                            image_tensor_save = torch.unsqueeze(copy.deepcopy(images_x0[0]).to(images_x0.device), 0)
                ...
                if lvm_guidance:
                    ...
                    # Forward to lvm
                        ...
                        for j in range(len(prompt)):
                            ...
                            conv.process_prompt_mode = process_prompt_mode
                            convs.append(conv)
                            prompt_i = prompt[j]
                            conv_i = convs[j]
                        ...
                        # print(images_x0.shape)
                        # 3. Forward through LVM
                        enhanced_prompts = []
                        for j in range(len(qs_prompts)):
                            qs_pr = qs_prompts[j]
                            images_x0 = torch.unsqueeze(images_x0[j], 0).to(lvm_model.device)
                            # print(type(images_x0))
                            input_ids = (
                            ...
                            enhanced_prompt = convs[j].post_process_prompt(generated_feedback)
                        ...
                        if track_image_prompt:
                            #save eh
                            for j in range(len(enhanced_prompts)):
                                f.write(f"enhanced prompt {i} _ {j}: {enhanced_prompts[j]} \n")
