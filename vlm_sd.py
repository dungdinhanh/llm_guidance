...
from diffusion.lvm_diffusion import StableDiffusionLVMPipeline
...
def main():
    ...
    pipe = StableDiffusionLVMPipeline.from_pretrained("CompVis/stable-diffusion-v1-4", cache_dir=cache_hub_folder)
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(
        args.model_path, None, model_name, device_map={"": accelerator.process_index}
    )
    ...
    for i in range(iterations):
        ...
        samples = pipe(prompt=prompts, guidance_scale=args.cfg_scale, skip=args.skip, lvm_tokenizer=tokenizer, lvm_model=model, lvm_image_processor=image_processor).images

        for i, sample in enumerate(samples):
            index = i * dist.get_world_size() + accelerator.process_index + total
