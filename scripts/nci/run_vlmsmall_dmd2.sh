...
#PBS -o output_nci/run_vlm_dmd2_small.txt
...
cmd="HUGGINGFACE_HUB_CACHE=${hub_folder} HFAI_DATASETS_DIR=/scratch/jp09/dd9648/data/ python3  vlm_dmd_shard_sk.py $MODEL_FLAGS  $SAMPLE_FLAGS --base_folder ${base_folder} \
 --sample-dir runs/exps/seed${seed}/vlm_dmd2_skip${skip}/  --seed ${seed} --lvm-guidance --model_path liuhaotian/llava-v1.5-7b --skip ${skip}"
echo ${cmd}
eval ${cmd}
done
done
...
do
  ...
  for skip in "${skips[@]}"
do
sample_file="${base_folder}runs/exps/seed${seed}/vlm_dmd2_skip${skip}/reference/samples_30000x256x256x3.npz"
