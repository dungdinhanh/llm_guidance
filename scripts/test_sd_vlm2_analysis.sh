...
SAMPLE_FLAGS="--per-proc-batch-size 1  --num-fid-samples 1 --fix_seed"
...
cmd="python analyse_vlm_sd_shard_negp.py  $MODEL_FLAGS  $SAMPLE_FLAGS \
 --sample-dir runs/analyse/seed${seed}/lvm_sd_test_negp_wkwn/ --cfg-scale ${scale} --lvm-guidance --seed ${seed} --prompt_process 0"
echo ${cmd}
eval ${cmd}
done
done
