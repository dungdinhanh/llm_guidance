...
SAMPLE_FLAGS="--per-proc-batch-size 3  --num-fid-samples 30000 --fix_seed"
...
ref_folder="/hdd/dungda/reference"
cmd="ls"
echo ${cmd}
eval ${cmd}
...
for scale in "${scales[@]}"
do
cmd="HUGGINGFACE_HUB_CACHE=${hub_folder} python dmd_shard.py  $MODEL_FLAGS  $SAMPLE_FLAGS --base_folder ${base_folder} \
 --sample-dir runs/exps/seed${seed}/dmd2_baseline/  --seed ${seed} "
echo ${cmd}
eval ${cmd}
done
done
...
do
  for scale in "${scales[@]}"
  do
sample_file="${base_folder}runs/exps/seed${seed}/dmd2_baseline/reference/samples_30000x256x256x3.npz"
